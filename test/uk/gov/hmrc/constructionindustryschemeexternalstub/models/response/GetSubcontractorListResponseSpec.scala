...
final class GetSubcontractorListResponseSpec extends AnyWordSpec with Matchers {
  ...
  "GetSubcontractorListResponse Json format" should {
    ...
    "read response JSON and parse subcontractors (including missing utr)" in {
      val json = Json.parse(
        """
          |{
          |  "subcontractors": [
      ...
      out.subcontractors.head.subbieResourceRef mustBe Some(1L)
    ...
    "write a response to JSON" in {
      ...
      ((json \ "subcontractors")(0) \ "subbieResourceRef").as[Long] mustBe 10L
